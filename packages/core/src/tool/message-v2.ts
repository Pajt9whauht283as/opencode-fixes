...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  for (const msg of input) {
    ...
    if (msg.info.role === "assistant") {
      ...
      // Guard: if the assistant message has parts but no renderable content
      // (only reasoning with no text or tool output), skip it to prevent
      // provider schema validation errors from empty/missing content fields.
      const hasRenderableContent = assistantMessage.parts.some(
        (p) => p.type === "text" || p.type.startsWith("tool-"),
      )
      if (assistantMessage.parts.length > 0 && !hasRenderableContent) {
        continue
      }
