...
describe("tool.edit", () => {
  ...
  describe("editing existing files", () => {
    ...
    it.instance("treats identical oldString and newString as no-op", () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const filepath = path.join(test.directory, "file.txt")
        yield* put(filepath, "content")

        const result = yield* run({ filePath: filepath, oldString: "same", newString: "same" })
  ...
  describe("edge cases", () => {
    ...
    it.instance("treats identical empty oldString and newString as no-op", () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const filepath = path.join(test.directory, "file.txt")
        yield* put(filepath, "content")

        // Empty strings are still rejected (different code path)
